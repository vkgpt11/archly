...
public class ProjectShareService {
    ...
    public ProjectShareService(ProjectRepository projects, ProjectShareRepository shares, ProjectService projectService) {
        this.projects = projects; this.shares = shares; this.projectService = projectService;
    }
    ...
    public ShareLinkResponse create(String email, UUID projectId, CreateShareRequest request) {
        ...
        ProjectShare share = shares.save(new ProjectShare(project, hash(token), request.permission()));
        return new ShareLinkResponse(share.getId(), token, share.getPermission(), false, share.getCreatedAt());
    }
    ...
    public List<ShareSummaryResponse> list(String email, UUID projectId) {
        ...
        return shares.findAllByProjectIdOrderByCreatedAtDesc(projectId).stream()
            .map(s -> new ShareSummaryResponse(s.getId(), s.getPermission(), s.isRevoked(), s.getCreatedAt(), s.getRevokedAt())).toList();
    }
    ...
    private ProjectShare active(String token) {
        return shares.findByTokenHashAndRevokedFalse(hash(token))
