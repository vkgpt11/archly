# Archly Documentation

This directory contains the product and technical documentation for Archly.

## Current documents

- [V1 product requirements](requirements-v1.md)
- [V2 improvement requirements](requirements-v2.md)
- [V3 production-readiness requirements](requirements-v3.md)
  - [V3 product analytics and administration](V3_product-analytics-and-administration.md)
- [V4 completion requirements and release criteria](requirements_v4.md)
  - [V4 portability: external documentation embeds](requirements_V4_portability.md)
- [External embedding operations](external-embedding-operations.md)
- [AI diagram generation requirements](requirements-ai-diagram-generation.md)
- [V1 canvas requirements](canvas-requirements-v1.md)
- [GCP deployment requirements](deployment-requirements-gcp.md)
- [GCP deployment configuration checklist](gcp-deployment-configuration.md)
- [Production environment rebuild guide](deployment-rebuild-guide.md)
- [Deployment frequently asked questions](deployment-faq.md)

## Planned documentation

- Product requirements and prototype scope
- System architecture and design decisions
- UI guidelines and user flows
- Backend API documentation
- Data model
- Deployment and operations guide
