...
export type DiagramCodeResult = { nodes: Node[]; edges: Edge[]; direction: LayoutOptions['direction']; variants: string[]; activeVariant?: string }
...
export function parseDiagramCode(source: string, activeVariant?: string): DiagramCodeResult {
  const compiled = compileVariantSource(source, activeVariant)
  const expanded = expandDiagramBlocks(expandDiagramTemplates(compiled.effectiveSource))
  try {
    ...
    return { ...result, ...applied, nodes: fitDiagramBoundaries(applied.nodes), variants: compiled.variants.map((item) => item.name), ...(activeVariant ? { activeVariant } : {}) }
  } catch (error) {
...
function parseExpandedDiagramCode(source: string): DiagramCodeResult {
  ...
  return { nodes, edges, direction, variants: [] }
}
