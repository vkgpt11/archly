...
export const languageProperties = new Set(['fill', 'border', 'text', 'description', 'icon', 'shape', 'opacity', 'border-width', 'width', 'height', 'padding', 'color', 'line', 'routing', 'start', 'end', 'protocol', 'port', 'async', 'encrypted', 'label', 'kind', 'replica-count', 'provider', 'identifier', 'type', 'x', 'y', 'horizontal-spacing', 'vertical-spacing', 'rank-separation', 'severity', 'suppress'])
const componentKinds = [...new Set(componentDefinitions.flatMap((item) => [item.kind, item.iconId]).filter(Boolean) as string[])]
const declarationKeywords = ['service', 'web', 'mobile', 'database', 'cache', 'queue', 'storage', 'external', 'actor', 'container', 'region', 'account', 'subscription', 'project', 'zone', 'vpc', 'vnet', 'subnet', 'cluster', 'namespace', 'connection', 'template', 'use', 'variant', 'let', 'class', 'import', 'view', 'rule']
...
export function completionsAt(source: string, offset: number, symbols: DiagramSymbol[]) {
  ...
  if (property === 'severity') return ['error', 'warning', 'info', 'off']
